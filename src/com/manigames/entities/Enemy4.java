...
for(int i =0; i <4; i++) {
			...
			rightEnemy[i] = Game.spritesheet.getSprite(0 + (i*32), 288, 32, 32);
			
		}
            for(int i =0; i <4; i++) {
			
			leftEnemy[i] = Game.spritesheet.getSprite(128 + (i*32), 160, 32, 32);
			
		}   for(int i =0; i <4; i++) {
			
			downEnemy[i] = Game.spritesheet.getSprite(128 + (i*32), 192 , 32, 32);
			
		}  for(int i =0; i <4; i++) {
			
			upEnemy[i] = Game.spritesheet.getSprite(0 + (i*32), 256, 32, 32);
	...
	public void render(Graphics g) {
		...
		g.drawImage(ENEMY_FEEDBACK4, this.getX() - Camera.x,this.getY() - Camera.y, null);
	}
